...
namespace Raytracer {
  ...
  void Api::set_hres(int resolution) {
    w->vp.set_hres(resolution);
  }

  void Api::set_vres(int resolution) {
    w->vp.set_vres(resolution);
  }

  void Api::set_number_of_samples(int samples) {
  ...
  void Api::set_background_color(QColor c) {
    w->background_color = RGBColor(c.redF(), c.greenF(), c.blueF());
  }

  void Api::set_ambient_light(Ambient* a) {
    w->set_ambient_light(a);
  }

  void Api::set_tracer(Tracer* tracer) {
    w->tracer_ptr = tracer;
  }

  void Api::set_tracer(QString s) {
  ...
  void Api::change_view_distance(double dd) {
    set_view_distance(w->camera_ptr->get_view_distance() + dd);
  }

  void Api::set_view_distance(double d) {
  ...
  const char* Api::get_camera_eye_as_string() {
    static char str[50];
  ...
  const char* Api::get_camera_eye_cylindrical_as_string() {
    static char str[50];
  ...
  const char* Api::get_camera_eye_spherical_as_string() {
    static char str[50];
  ...
  void Api::camera_eye_absolute(Vector3d v) {
    w->camera_ptr->set_eye(v);
    w->camera_ptr->compute_uvw();
  }

  void Api::camera_eye_absolute(double x, double y, double z) {
    camera_eye_absolute(Vector3d(x, y, z));
  }

  void Api::camera_eye_relative(double dx, double dy, double dz) {
    Vector3d v = w->camera_ptr->get_eye();
    camera_eye_absolute(v(0) + dx, v(1) + dy, v(2) + dz);
  }

  void Api::camera_eye_relative_cylindrical(double dp, double dphi, double dz) {
    Vector3d cc = absolute_to_cylindrical(w->camera_ptr->get_eye());
    cc(0) += dp;
  ...
  void Api::camera_eye_relative_spherical(double dr, double dphi, double dtheta) {
    Vector3d ss = absolute_to_spherical(w->camera_ptr->get_eye());
    ss(0) += dr;
    ss(1) = ss(1) + dphi;
    ss(2) = ss(2) + dtheta;
  ...
  Vector3d Api::cylindrical_to_absolute(Vector3d c) {
    Vector3d a;
    a(0) = c(0) * cos(c(1));
    a(1) = c(0) * sin(c(1));
    a(2) = c(2);
  ...
  Vector3d Api::absolute_to_cylindrical(Vector3d a) {
    Vector3d c;
  ...
  Vector3d Api::spherical_to_absolute(Vector3d s) {
    Vector3d a;
    // s(0) = r
    // s(1) = phi
    // s(2) = theta
  ...
  Vector3d Api::absolute_to_spherical(Vector3d a) {
    // s(0) = r
    // s(1) = phi
    // s(2) = theta
    // a(0) = x, a(1) = y, a(2) = z
  ...
  inline double Api::clamp(double x, double max) {
  ...
  void Api::set_up_axis_matte() {
    ...
    Raytracer::Sphere* sphere1 = new Raytracer::Sphere(Vector3d(250.0, 0.0, 0.0), 30.0);
    sphere1->set_material(Matte::dummy(light_green));
    w->add_object(sphere1);

    Raytracer::Sphere*	sphere2 = new Raytracer::Sphere(Vector3d::Zero(), 15.0);
    sphere2->set_material(Matte::dummy(white));
    w->add_object(sphere2);

    Raytracer::Sphere*	sphere3 = new Raytracer::Sphere(Vector3d(-2 * dx, 0.0, 0.0), dx);
    sphere3->set_material(Matte::dummy(light_green));
    w->add_object(sphere3);

    Raytracer::Sphere*	sphere4 = new Raytracer::Sphere(Vector3d(2 * dx, 0.0, 0.0), dx);
    sphere4->set_material(Matte::dummy(dark_green));
    w->add_object(sphere4);

    Raytracer::Sphere*	sphere6 = new Raytracer::Sphere(Vector3d(0.0, -2 * dx, 0.0), dx);
    sphere6->set_material(Matte::dummy(light_grey));
    w->add_object(sphere6);


    Raytracer::Sphere*	sphere7 = new Raytracer::Sphere(Vector3d(0.0, 2 * dx, 0.0), dx);
    ...
    Raytracer::Sphere*	sphere8 = new Raytracer::Sphere(Vector3d(0.0, 0.0, -2 * dx), dx);
    sphere8->set_material(Matte::dummy(light_purple));
    w->add_object(sphere8);

    Raytracer::Sphere*	sphere9 = new Raytracer::Sphere(Vector3d(0.0, 0.0, 2 * dx), dx);
    sphere9->set_material(Matte::dummy(dark_purple));
    w->add_object(sphere9);

    Raytracer::Plane* plane0 = new Raytracer::Plane(Vector3d::Zero(), Vector3d(0.0, 1.0, 0.0));
  ...
  void Api::set_up_axis_phong() {
    ...
    Raytracer::Plane* plane1 = new Raytracer::Plane(Vector3d(0.0, 100.0, 0.0), Vector3d(0.0, 1.0, 0.0));
